# this script runs PCA + Random Forest on preprocessed metabolomic data
# POSITIVE CLASS = 1 (GT_present)
# UPDATED: Only runs RF on PCA-reduced features with AUC calculations
...
###########################################################################
# PCA + RF MODEL TRAINING FUNCTION
###########################################################################

train_pca_rf_model <- function(train_data, target_col = "gt", n_components = 10) {
  
  cat("=== TRAINING PCA + RF MODEL ===\n")
  ...
  # Perform PCA
  cat("Performing PCA with", n_components, "components...\n")
  pca_model <- prcomp(x_train, center = TRUE, scale. = TRUE)
  
  # Extract specified number of components
  pca_train <- as.data.frame(pca_model$x[, 1:n_components])
  pca_train[[target_col]] <- y_train
  
  # Calculate variance explained
  var_explained <- summary(pca_model)$importance[2, 1:n_components]
  total_var_explained <- sum(var_explained)
  cat("Total variance explained by", n_components, "PCs:", round(total_var_explained * 100, 1), "%\n")
  
  # Random Forest with hyperparameter tuning on PCA data
  cat("Training Random Forest on PCA components...\n")
  
  # Determine mtry range for PCA components
  max_mtry <- min(n_components, 5)  # Cap at 5 for PCA
  
  if(n_components >= 3) {
    mtry_values <- unique(c(1, floor(sqrt(n_components)), floor(n_components/2), max_mtry))
    mtry_values <- mtry_values[mtry_values <= n_components & mtry_values >= 1]
  } else {
    mtry_values <- 1:n_components
  }
  ...
  pca_train_caret <- pca_train
  pca_train_caret[[target_col]] <- factor(pca_train_caret[[target_col]], 
                                         levels = c(0, 1), 
  ...
  # Train with tuning using ranger
  rf_tuned <- train(
    as.formula(paste(target_col, "~ .")),
    data = pca_train_caret,
    method = "ranger",
  ...
  cat("PCA + RF model trained successfully\n\n")
  ...
  return(list(
    ...
    var_explained = total_var_explained,
    best_rf_params = list(mtry = best_mtry, nodesize = best_nodesize)
  ))
}

###########################################################################
# PCA + RF MODEL EVALUATION FUNCTION
###########################################################################

evaluate_pca_rf_on_test <- function(models, test_data, target_col = "gt") {
  
  cat("=== EVALUATING PCA + RF ON TEST SET ===\n")
  ...
  pca_test <- as.data.frame(predict(models$pca_model, x_test)[, 1:models$n_components])
  pca_test[[target_col]] <- y_test
  
  # Get RF predictions (probabilities)
  rf_pred_prob <- predict(models$rf_model, pca_test, type = "prob")[,2]  # Class "1" probabilities
  
  # Calculate AUC
  rf_auc <- auc(y_test, rf_pred_prob, quiet = TRUE)
  
  # Calculate other metrics at optimal threshold (0.5)
  ...
  # Confusion matrix
  cm <- confusionMatrix(rf_pred_class, actual, positive = "1")
  ...
  results <- list(
    ...
    var_explained = models$var_explained
  )
  
  cat(sprintf("PCA_RF - AUC: %.3f, Accuracy: %.3f, Balanced Acc: %.3f\n", 
              results$auc, results$accuracy, results$balanced_accuracy))
  cat(sprintf("Variance explained: %.1f%%\n", models$var_explained * 100))
  
  return(list(
    results = results,
    rf_params = models$best_rf_params,
    predictions = rf_pred_prob
  ))
}

###########################################################################
# MAIN EVALUATION FUNCTION FOR PCA + RF
###########################################################################

evaluate_pca_rf <- function(data, target_col = "gt", test_prop = 0.3, seed = 123, n_components = 10) {
  ...
  cat("Original features:", ncol(data) - 1, "-> PCA components:", n_components, "\n\n")
  
  # Train PCA + RF model
  models <- train_pca_rf_model(train_data, target_col, n_components)
  
  # Evaluate on test set
  results <- evaluate_pca_rf_on_test(models, test_data, target_col)
...
###########################################################################
# CROSS-VALIDATION FUNCTION FOR PCA + RF WITH AUC AND SE
###########################################################################

run_cv_pca_rf <- function(data, target_col = "gt", n_runs = 10, test_prop = 0.3, n_components = 10) {
  
  cat("=== RUNNING", n_runs, "RUNS FOR PCA + RF WITH AUC ===\n")
  cat("PCA Components:", n_components, "\n\n")
  ...
  for(i in 1:n_runs) {
    ...
    # Extract metrics
    metrics <- result$results$results
    all_results$accuracy[i] <- metrics$accuracy
    all_results$balanced_accuracy[i] <- metrics$balanced_accuracy
    all_results$auc[i] <- metrics$auc
    all_results$var_explained[i] <- metrics$var_explained
    ...
    cat(" AUC:", round(metrics$auc, 3),
        ...
        " VarExp:", round(metrics$var_explained * 100, 1), "%\n")
  }
  
  # Calculate summary statistics with AUC and SE
  # Remove NA values
  acc_vals <- all_results$accuracy[!is.na(all_results$accuracy)]
  bal_acc_vals <- all_results$balanced_accuracy[!is.na(all_results$balanced_accuracy)]
  auc_vals <- all_results$auc[!is.na(all_results$auc)]
  var_exp_vals <- all_results$var_explained[!is.na(all_results$var_explained)]
  ...
  summary_stats <- data.frame(
    ...
    Mean_Var_Explained = round(mean(var_exp_vals) * 100, 1),
    SD_Var_Explained = round(sd(var_exp_vals) * 100, 1),
    stringsAsFactors = FALSE
  )
  
  # Print summary with AUC
  cat("\n=== SUMMARY STATISTICS FOR PCA + RF ===\n")
  print(summary_stats)
  
  # Create comparison table
  cat("\n=== PCA + RF RESULTS (AUC ± SE) ===\n")
  comparison_table <- data.frame(
    ...
    Var_Explained = paste0(summary_stats$Mean_Var_Explained, "% ± ", summary_stats$SD_Var_Explained, "%"),
    stringsAsFactors = FALSE
  )
  
  print(comparison_table)
  
  # Statistical significance test (vs AUC = 0.5)
  ...
  if(length(auc_vals) > 1) {
    ...
    cat(sprintf("PCA_RF: AUC = %.3f ± %.3f (vs 0.5, p = %.4f, %s)\n", 
                mean_auc, sd_auc, t_test$p.value, sig_text))
  }
  ...
  detailed_results <- data.frame(
    ...
    Balanced_Accuracy = all_results$balanced_accuracy,
    AUC = all_results$auc,
    Var_Explained = all_results$var_explained * 100,
    RF_mtry = mtry_values,
...
# Run PCA + RF evaluation with 10 components
cv_results <- run_cv_pca_rf(boost_data, target_col = "gt", n_runs = 10, test_prop = 0.3, n_components = 10)

# Save results
write.csv(cv_results$summary_stats, "../results/metab_rf/pca_rf_summary_cv_with_auc.csv", row.names = FALSE)
write.csv(cv_results$detailed_results, "../results/metab_rf/pca_rf_detailed_cv_with_auc.csv", row.names = FALSE)
write.csv(cv_results$comparison_table, "../results/metab_rf/pca_rf_comparison_table_auc.csv", row.names = FALSE)

cat("\nPCA + RF results with AUC ± SE saved to CSV files\n")
