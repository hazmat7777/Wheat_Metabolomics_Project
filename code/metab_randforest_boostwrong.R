...
for (i in 1:20) {
  ...
  rf_pred <- predict(res$models$rf, res$test_data, type = "response")
  gbm_pred <- predict(res$models$gbm, res$test_data, 
                              n.trees = res$models$gbm$n.trees, type = "response")
  en_pred <- predict(res$models$glmnet, as.matrix(res$test_data[,-ncol(res$test_data)]), 
                             s = "lambda.min", type = "response")[,1]
  
  # make them numeric (rn they're factors)
  rf_pred <- as.numeric(rf_pred)
  gbm_pred <- as.numeric(gbm_pred)
  en_pred <- as.numeric(en_pred)
...
res
length(rf_pred)  # Number of predictions from Random Forest
length(gbm_pred)  # Number of predictions from GBM
length(en_pred)  # Number of predictions from ElasticNet
length(test_labels)  # Number of test labels
rf_pred
gbm_pred
