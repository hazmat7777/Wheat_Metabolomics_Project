...
"PHENYLALANINE") # need to add griseofulvin et al, also that precursor to 24DAPG. And remove some with tenuous antifungal activity?

# load data
fk_metabolom_gt_log <- readRDS("../data/metabolomics/fk_metabolomics_gt_logged.RDS")
View(fk_metabolom_gt_log)
...
available_antifungal <- intersect(interesting_compounds, colnames(fk_metabolom_gt_log))
...
antifungal_data <- fk_metabolom_gt_log[, c(available_antifungal, "gt")]
View(antifungal_data)

# Make the predictors numeric
...
# Scale the data- not doing this as it's logged
