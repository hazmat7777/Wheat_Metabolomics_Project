...
# old code
# # Panel 1: Metabolite diversity (number of detected antifungal compounds) vs GT
# # Calculate metabolite diversity per sample
# metabolite_diversity <- antifungal_data %>%
#   rowwise() %>%
#   mutate(
#     metabolite_abundance = sum(c_across(all_of(available_antifungal)), na.rm = TRUE)
...
# # Metabolite diversity boxplot
# p1 <- ggplot(metabolite_diversity, aes(x = gt, y = metabolite_abundance, fill = gt)) +
#   geom_boxplot(alpha = 0.7, outlier.shape = NA) +
#   geom_jitter(width = 0.2, alpha = 0.6, size = 2) +
#   labs(
#     x = "GT Status",
#     y = "Number of Detected Antifungal Metabolites"
#   ) +
...
# # Panel 2: Bacterial richness vs GT
# p2 <- ggplot(ps_fs_div_df, aes(x = gt, y = Observed, fill = gt)) +
#   geom_boxplot(alpha = 0.7, outlier.shape = NA) +
#   geom_jitter(width = 0.2, alpha = 0.6, size = 2) +
#   labs(
#     x = "GT Status",
#     y = "Observed Richness of Pseudomonas and Bacillus"
#   ) +
#   scale_fill_manual(values = c("GT_absent" = "#E69F00", "GT_present" = "#56B4E9")) +
#   theme_classic() +
#   theme(
...
# # Save the figure
# ggsave("metabolite_bacterial_gt_comparison.png", combined_plot, 
#        width = 12, height = 5, dpi = 300)

# # Print the combined plot
# combined_plot

# ggsave("../results/plots/feature_selection_metab_microb_gt.png", combined_plot, 
#        width = 12, height = 5, dpi = 300)