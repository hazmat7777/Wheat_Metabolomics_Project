...
# first source the pca rf script
...
# Extract PCA RF model and test data
rf_pca <- pca_results$models$rf
test_pca_df <- pca_results$test_data
n_components <- pca_results$preprocessing_params$n_components  # Optional, for tracking

# Predict and evaluate
pred_pca <- predict(rf_pca, test_pca_df)
cm_pca <- confusionMatrix(pred_pca, factor(test_pca_df$gt, levels = c(0, 1)), positive = "1")

# Calculate AUC
pred_pca_prob <- predict(rf_pca, test_pca_df, type = "prob")
roc_pca <- pROC::roc(test_pca_df$gt, pred_pca_prob[, 2])
auc_pca <- pROC::auc(roc_pca)
