...
# now featuring SMOTE
# SMOTE isnt helping. the real problem is the small sample size- 6 samples.
...
run_improved_rf <- function(feature_data, response_var, approach_name, 
  ...
  cat("Original response distribution:", table(rf_data$response), "\n")
  ...
  if (use_smote && nrow(train_data) > 10) {  # Only use SMOTE if enough samples
    cat("Applying SMOTE to balance training data...\n")
    
    # Calculate minority class size
    ...
    if (max_class_size > min_class_size) {
      # Prepare data for smotefamily (needs numeric features and factor response)
      smote_features <- train_data[, -ncol(train_data)]  # Remove response column
      smote_response <- train_data$response
      
      # Apply SMOTE using smotefamily
      smote_result <- SMOTE(smote_features, smote_response, K = 5)
      
      # Extract the balanced dataset
      train_data_smote <- smote_result$data
      colnames(train_data_smote)[ncol(train_data_smote)] <- "response"  # Rename class column
      train_data_smote$response <- as.factor(train_data_smote$response)
      
      cat("SMOTE applied - New training distribution:", table(train_data_smote$response), "\n")
      train_data <- train_data_smote
    } else {
      cat("Classes already balanced, skipping SMOTE\n")
    }
  } else {
    cat("Skipping SMOTE (insufficient samples or disabled)\n")
  }
  ...
  # Set up cross-validation with balanced sampling
  ctrl <- trainControl(
    method = "cv",
    number = cv_folds,
    classProbs = TRUE,
    summaryFunction = twoClassSummary,
    sampling = "down"  # Downsample majority class in CV
  )
  ...
  final_rf <- randomForest(
    ...
    ntree = 500,
    importance = TRUE,
    classwt = c("GT_absent" = 1, "GT_present" = 1)  # Equal class weights
  )
  ...
  # Confusion matrix
  cm <- confusionMatrix(test_pred, test_data$response, positive = "GT_present")
  ...
  cat("Test Sensitivity (GT_present detection):", round(cm$byClass["Sensitivity"] * 100, 2), "%\n")
  cat("Test Specificity (GT_absent detection):", round(cm$byClass["Specificity"] * 100, 2), "%\n")
  cat("Balanced Accuracy:", round(cm$byClass["Balanced Accuracy"] * 100, 2), "%\n")
